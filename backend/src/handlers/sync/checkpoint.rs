...
use crate::services::etapa1_sync::{sync_checkpoint, SyncRequestBody};
use crate::solana::SolanaRpcClient;

#[rocket::post("/checkpoints/sync", format = "json", data = "<body>")]
pub async fn post_sync_checkpoint(
    pool: &State<PgPool>,
    ...
    match sync_checkpoint(pool.inner(), rpc.inner(), &cfg.program_id, &body).await {
        Ok(o) => {
