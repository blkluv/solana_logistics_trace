...
use crate::services::etapa1_sync::{sync_actor, SyncRequestBody};
use crate::solana::SolanaRpcClient;

#[rocket::post("/actors/sync", format = "json", data = "<body>")]
pub async fn post_sync_actor(
    pool: &State<PgPool>,
    ...
    match sync_actor(pool.inner(), rpc.inner(), &cfg.program_id, &body).await {
        Ok(o) => {
