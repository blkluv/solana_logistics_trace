...
use crate::services::etapa1_sync::{sync_assign_carrier, SyncRequestBody};
use crate::solana::SolanaRpcClient;

#[rocket::post("/shipments/assign-carrier/sync", format = "json", data = "<body>")]
pub async fn post_sync_assign_carrier(
    pool: &State<PgPool>,
    ...
    match sync_assign_carrier(pool.inner(), rpc.inner(), &cfg.program_id, &body).await {
        Ok(o) => Ok((
            if o.created {
                Status::Created
            } else {
